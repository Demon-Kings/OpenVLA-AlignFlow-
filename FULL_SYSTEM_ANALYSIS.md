...
# 九、 简历精简 STAR 描述与面试高频答辩话术

### 📄 简历精简版（4 条标准 Bullet Points）
- **连续动作流架构构建**：针对传统离散 VLA 动作延迟与均值坍塌痛点，设计基于 **Qwen3-VL / SigLIP 底座与条件流匹配（CFM）连续动作头**，通过 **$k=16$ 时序动作块与 4 步 Euler ODE 极速积分**，将轨迹预测 L1 误差下降 **60.0%**，推理速度较传统 Diffusion 提升 90% 以上。
- **具身细粒度图文对齐**：提出 **阶段目标对比学习（Sub-goal InfoNCE）与空间可供性注意力损失（Affordance Mask Loss）**，引导模型 Cross-Attention 精准聚焦机械臂接触与可操作部件，使空间注意力重合度（IoU）由 1.1% 提升至 **75.2%**。
- **高阶轨迹 DPO 离线对齐**：将大模型 SOTA DPO 创新性迁移至连续动作流，引入 **柯西 $C^1$ 光滑 BNF、KKT 动态对偶 $\beta$ 与轨迹时空效率控长**，彻底消除动作硬阶跃与空中无效停顿，将机械臂末端**物理加加速度（Jerk）稳定压制至 $11.60\text{ m/s}^3$**（达真实机械臂平滑控制标准）。
- **具身数据动力学清洗**：基于 OpenX / BridgeData v2 构建一阶/二阶导数**动力学滤波器（Kinetic Filter）**，统一规范化 7-DoF 末端相对位姿并自动清洗剔除 18.5% 的遥操作抖动数据，全流程自动化构建高质量偏好对。
